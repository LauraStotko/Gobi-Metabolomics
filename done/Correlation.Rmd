---
title: "Pairwise Pearson Correlation Analysis"
date: "`r Sys.Date()`"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```


## 1. Bibliotheken laden
```{r}
library(data.table)
library(ggplot2)
library(dplyr)
library(stats)
library(tidyverse)
library(pheatmap)
```

## 2. Daten einlesen
```{r}
df <- fread("/Users/laura.stotko/Downloads/postprandial_imputed.csv")
head(df)
```

## **Korrelation berechnen**

```{r correlation}
str(df)
```

## **Korrelation berechnen**

```{r correlation}
# Identifiziere Metaboliten-Spalten (alle numerischen Spalten außer Insulin)
metabolite_columns <- names(df)[sapply(df, is.numeric)] # Alle numerischen Spalten
metabolite_columns <- setdiff(metabolite_columns, c("insulin_biochemistry", "V1", "subject", "challenge_time", "challenge"))

# Berechnung der paarweisen Pearson-Korrelation zwischen Insulin und allen Metaboliten
cor_results <- data.frame(Metabolite = metabolite_columns, Correlation = NA, p_value = NA)

for (metabolite in metabolite_columns) {
  cor_test <- cor.test(df[[metabolite]], df$insulin_biochemistry, method = "pearson")
  cor_results[cor_results$Metabolite == metabolite, "Correlation"] <- cor_test$estimate
  cor_results[cor_results$Metabolite == metabolite, "p_value"] <- cor_test$p.value
}

# Multiple Testing Correction (Bonferroni)
cor_results$p_adjusted <- p.adjust(cor_results$p_value, method = "bonferroni")

# Ergebnisse anzeigen
cor_results <- cor_results %>% arrange(p_adjusted)
print(head(cor_results, 10)) # Zeige die Top 10 signifikanten Korrelationen
```
```{r save-results}
write.csv(cor_results, "/Users/laura.stotko/Downloads/correlation_results.csv", row.names = FALSE)
message("Ergebnisse gespeichert in 'correlation_results.csv'")
```
## **Visualisierung der Ergebnisse**

```{r plot-correlation, warning=FALSE}
library(ggplot2)

# Scatterplot für die Top-korrelierte Variable
top_metabolite <- cor_results$Metabolite[1]

# Nutze .data[[...]] für sichere Spaltennamen in ggplot2
ggplot(df, aes(x = .data[[top_metabolite]], y = .data[["insulin_biochemistry"]])) +
  geom_point(alpha = 0.6, color = "blue") +
  geom_smooth(method = "lm", col = "red") +
  labs(title = paste("Korrelation zwischen", top_metabolite, "und Insulin"),
       x = top_metabolite, y = "Insulin (biochemistry)") +
  theme_minimal()
```


---
title: "Pairwise Pearson Correlation Analysis"
author: "Dein Name"
date: "`r Sys.Date()`"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Einleitung
In diesem Dokument wird die **pairwise Pearson correlation** zwischen den Metaboliten und Insulin-Werten berechnet.

## **Daten einlesen**

```{r load-data}
library(data.table)
library(tidyverse)

# Datei einlesen
df <- fread("/Users/laura.stotko/Downloads/postprandial_imputed.csv")

# Datenstruktur anzeigen
str(df)
```

## **Korrelation berechnen**

```{r correlation}
# Identifiziere Metaboliten-Spalten (alle numerischen Spalten außer Insulin)
metabolite_columns <- names(df)[sapply(df, is.numeric)] # Alle numerischen Spalten
metabolite_columns <- setdiff(metabolite_columns, c("insulin_biochemistry", "V1", "subject", "challenge_time", "challenge"))

# Berechnung der paarweisen Pearson-Korrelation zwischen Insulin und allen Metaboliten
cor_results <- data.frame(Metabolite = metabolite_columns, Correlation = NA, p_value = NA)

for (metabolite in metabolite_columns) {
  cor_test <- cor.test(df[[metabolite]], df$insulin_biochemistry, method = "pearson")
  cor_results[cor_results$Metabolite == metabolite, "Correlation"] <- cor_test$estimate
  cor_results[cor_results$Metabolite == metabolite, "p_value"] <- cor_test$p.value
}

# Multiple Testing Correction (Bonferroni)
cor_results$p_adjusted <- p.adjust(cor_results$p_value, method = "bonferroni")

# Ergebnisse anzeigen
cor_results <- cor_results %>% arrange(p_adjusted)
print(head(cor_results, 10)) # Zeige die Top 10 signifikanten Korrelationen
```

## **Visualisierung der Ergebnisse**

```{r plot-correlation, warning=FALSE}
library(ggplot2)

# Scatterplot für die Top-korrelierte Variable
top_metabolite <- cor_results$Metabolite[1]

# Nutze .data[[...]] für sichere Spaltennamen in ggplot2
ggplot(df, aes(x = .data[[top_metabolite]], y = .data[["insulin_biochemistry"]])) +
  geom_point(alpha = 0.6, color = "blue") +
  geom_smooth(method = "lm", col = "red") +
  labs(title = paste("Korrelation zwischen", top_metabolite, "und Insulin"),
       x = top_metabolite, y = "Insulin (biochemistry)") +
  theme_minimal()
```

### **Heatmap der Korrelationen**

```{r heatmap-correlation, warning=FALSE}
library(pheatmap)

# Wähle die Top 20 korrelierten Metaboliten für die Heatmap
top_20_metabolites <- cor_results$Metabolite[1:20]

# Sicherstellen, dass die Daten korrekt extrahiert werden
cor_data <- df[, ..top_20_metabolites]  # Verwende data.table Syntax
cor_data <- as.matrix(cor_data)  # In Matrix umwandeln

# Berechnung der Korrelationsmatrix
cor_matrix <- cor(cor_data, use = "pairwise.complete.obs")

# Zeichne die Heatmap
pheatmap(cor_matrix, cluster_rows = TRUE, cluster_cols = TRUE, display_numbers = TRUE, 
         main = "Heatmap der Korrelationen", color = colorRampPalette(c("blue", "white", "red"))(100))
```

### **Super-Pathway Korrelationen (angepasst aus Supplementary Figure 2)**

```{r super-pathway-correlation, warning=FALSE}
library(ggplot2)

# Dummy-Daten: Falls verfügbar, lade eine Mapping-Tabelle, die Metaboliten zu Super-Pathways zuordnet
# Hier wird angenommen, dass es eine Spalte "Super_Pathway" gibt, die den Metaboliten zugeordnet ist.
# Falls nicht vorhanden, muss sie ergänzt werden.

# Füge eine Spalte mit der Signifikanzmarkierung hinzu
cor_results$Significant <- ifelse(cor_results$p_adjusted < 0.05, "yes", "no")

# Falls eine Mapping-Tabelle existiert, merge sie mit den Korrelationsergebnissen
# mapping <- read.csv("pathway_mapping.csv") # Datei mit Metaboliten und zugehörigen Super-Pathways
# cor_results <- merge(cor_results, mapping, by = "Metabolite", all.x = TRUE)

# Beispielweise manuelle Zuordnung von Dummy-Kategorien (falls keine Mapping-Datei vorhanden ist)
set.seed(123)  # Für reproduzierbare Zufallswerte
cor_results$Super_Pathway <- sample(c("Amino Acids", "Carbohydrates", "Lipids", "Nucleotides", "Peptides", "Xenobiotics", "Cofactors and Vitamins", "Energy"), nrow(cor_results), replace = TRUE)

# Erstelle das Plot
ggplot(cor_results, aes(x = Correlation, y = Super_Pathway, color = Significant)) +
  geom_point(alpha = 0.6) +
  scale_color_manual(values = c("yes" = "darkblue", "no" = "lightblue")) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  labs(title = "Pearson correlation between metabolites and insulin",
       x = "Pearson correlation between metabolites and insulin",
       y = "Super-Pathway") +
  theme_minimal()
```

