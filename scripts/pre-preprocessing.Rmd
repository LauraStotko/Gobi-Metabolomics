...
library(doParallel)
```
## Notiz: Ich benutze die drei verschiedenen wie gehabt, ohne challenge column, platform>platform_name. Anna transformiert und führt sie danach zusammen.#todo
...
# TOGGLE uncomment to filter out irrelevant time points before analysis
...
mF_function <- function(dataset) {
  ...
  if(!sum(is.na(dataset)) == 0) {
    ...
    imputed_dataset <- missForest(dataset, ntree = 3, parallelize = "variables", verbose = "TRUE") # default ntree = 100; verbose=True to get progress messages              # ACHTUNG: DAUERT LANGE!!
...
#imputed_inhouse_filtered_first <- mF_function(data_inhouse_filtered_first_first)
...
log2_imputed_data_metabolon_filtered_last <- log2_imputed_data_metabolon_filtered_last
# data_biocrates_filtered_last <- log2_imputed_data_biocrates_filtered_last
# log2_imputed_data_inhouse_filtered_last <- log2_imputed_data_inhouse_filtered_last
```
