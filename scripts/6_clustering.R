...
set.seed(42)
wss <- sapply(3:10, function(k){
  kmeans(df_zscore[, -1], centers = k, nstart = 10)$tot.withinss
})

# Plot der Elbow-Methode
plot(3:10, wss, type="b", pch = 19, frame = FALSE, col = "blue",
...
write.csv(df_clusters_full, "/Users/laura.stotko/Downloads/sup_table_7_fuzzy_cmeans_clustering.csv", row.names = FALSE)
head(df_clusters_full)
unique(df_long$time_point)
unique(df_long$challenge)
