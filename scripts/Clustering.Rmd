...
if("metabolite" %in% colnames(info_df)) {
    metabolite_mapping <- info_df %>% select(metabolite)
    
    # Metaboliten-Spalten extrahieren
    metabolites <- colnames(df)
    
    # Nur relevante Spalten umbenennen
    new_names <- metabolite_mapping$metabolite[match(metabolites, metabolite_mapping$metabolite)]
    
    # Ersetzen der Spaltennamen
    colnames(df)[!is.na(new_names)] <- new_names[!is.na(new_names)]
}
```
